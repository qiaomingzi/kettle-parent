...
public class StaxPoiSheet implements KSheet {
  ...
  private KCellType getCellType( String cellType, String cellStyle, boolean isFormula ) {
    ...
    if(cellType.equals("s"))  return KCellType.LABEL;
    if(cellType.equals("b"))  return isFormula ? KCellType.BOOLEAN_FORMULA : KCellType.BOOLEAN;
    if(cellType.equals("e"))  return KCellType.EMPTY;
    else  return KCellType.STRING_FORMULA;
   /* switch ( cellType ) {
      case "s":
